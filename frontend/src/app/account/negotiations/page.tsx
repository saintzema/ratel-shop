                ...
                <div className="space-y-4">
                    {filteredNegs.length === 0 ? (
                        <div className="text-center py-20 bg-gray-50 rounded-3xl border border-dashed border-gray-200">
                            ...
                            <Link href="/">
                                <Button className="mt-4 rounded-full font-bold">Start Shopping</Button>
                            </Link>
                        </div>
                    ) : (
                        filteredNegs.map((neg) => {
                            ...
                            return (
                                <div key={neg.id} className="bg-white border border-gray-200 rounded-2xl p-4 sm:p-6 shadow-sm hover:shadow-md transition-shadow">
                                    <div className="flex flex-col sm:flex-row gap-4 sm:gap-6">
                                        {/* Product Image */}
                                        <div className="w-20 h-20 sm:w-24 sm:h-24 bg-gray-50 rounded-xl flex items-center justify-center p-2 shrink-0 mx-auto sm:mx-0">
                                        ...
                                        <div className="flex-1 min-w-0">
                                            <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-2 sm:gap-4 mb-4">
                                                <div className="min-w-0">
                                                    <Link href={`/product/${product.id}`} className="font-bold text-base sm:text-lg hover:underline text-gray-900 line-clamp-2">
                                                        {product.name}
                                                    </Link>
                                                ...
                                                <div className="flex items-center gap-2 shrink-0">
                                                    {neg.status === "pending" && !isCounterOffer && (
                                                        <Badge className="bg-yellow-100 text-yellow-800 hover:bg-yellow-100 border-none text-[10px] sm:text-xs">Pending</Badge>
                                                    )}
                                                    {isCounterOffer && neg.status === "pending" && (
                                                        <Badge className="bg-blue-100 text-blue-800 hover:bg-blue-100 border-none animate-pulse text-[10px] sm:text-xs">Counter Offer</Badge>
                                                    )}
                                                    {neg.status === "accepted" && (
                                                        <Badge className="bg-emerald-100 text-emerald-800 hover:bg-emerald-100 border-none text-[10px] sm:text-xs">Accepted</Badge>
                                                    )}
                                                    {neg.status === "rejected" && (
                                                        <Badge className="bg-red-100 text-red-800 hover:bg-red-100 border-none text-[10px] sm:text-xs">Rejected</Badge>
                                                    )}
                                                </div>
                                            </div>

                                            {/* Price Details */}
                                            <div className="flex flex-wrap items-center gap-4 sm:gap-6 p-3 sm:p-4 bg-gray-50 rounded-xl border border-gray-100">
                                                <div>
                                                ...
                                                {(neg.counter_price || isCounterOffer) && (
                                                    <div className="flex-1 border-l pl-4 sm:pl-6 border-gray-200">
                                                        <div className="text-[10px] sm:text-xs text-blue-600 uppercase font-bold mb-1">Counter Offer</div>
                                                        <div className="flex items-baseline gap-2 flex-wrap">
                                                            <span className="text-lg sm:text-xl font-black text-blue-700">
                                                                {formatPrice(neg.counter_price || 0)}
                                                            </span>
                                                            {neg.counter_message && (
                                                                <span className="text-xs sm:text-sm text-gray-600 italic">&quot;{neg.counter_message}&quot;</span>
                                                            )}
                                                        </div>
                                                    </div>
                                                )}
                                            ...
                                            <div className="mt-4 sm:mt-6 flex flex-wrap gap-2 sm:gap-3">
                                                {isCounterOffer && (
                                                    <>
                                                        <Button
                                                            ...
                                                            className="bg-emerald-600 hover:bg-emerald-700 text-white font-bold rounded-full text-xs sm:text-sm h-9 sm:h-10"
                                                        >
                                                        ...
                                                        <Button
                                                            ...
                                                            variant="outline"
                                                            className="border-red-200 text-red-600 hover:bg-red-50 rounded-full font-bold text-xs sm:text-sm h-9 sm:h-10"
                                                        >
                                                            Reject
                                                        </Button>
                                                    </>
                                                ...
                                                {neg.status === "rejected" && (
                                                    <Link href={`/product/${product.id}`}>
                                                        <Button variant="outline" className="rounded-full text-xs sm:text-sm h-9 sm:h-10">View Product</Button>
                                                    </Link>
                                                )}
                                            </div>
                                        </div>
