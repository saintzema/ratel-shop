...
import {
    ...
    ChevronDown, Send, Download, Share2
} from "lucide-react";
                ...
                <DialogContent className="max-w-md rounded-3xl p-6 bg-white border-none shadow-2xl">
                    ...
                    {previewCard && (
                        <div className="mt-2">
                            ...
                            />
                            {(() => {
                                const shareText = `🎁 You've received a FairPrice Gift Card!\n\n💳 Code: ${previewCard.code}\n💰 Value: ₦${previewCard.value.toLocaleString()}\n\nRedeem it at checkout on ${typeof window !== "undefined" ? window.location.origin : "https://www.fairprice.ng"} — just enter the code in the discount field.${previewCard.expiry ? `\n\n⏳ Expires: ${new Date(previewCard.expiry).toLocaleDateString()}` : ""}`;
                                return (
                                    <div className="mt-4 space-y-2">
                                        <button
                                            onClick={() => handleCopy(previewCard.code)}
                                            className="w-full h-10 rounded-xl bg-gray-900 text-white font-black text-xs flex items-center justify-center gap-2"
                                        >
                                        ...
                                        <div className="flex gap-2">
                                            ...
                                            <a
                                                ...
                                                className="flex-1 h-10 rounded-xl bg-[#25D366] text-white font-black text-xs flex items-center justify-center gap-2"
                                            >
                                            ...
                                            {/* Native share (other socials / SMS / email) */}
                                            <button
                                                onClick={async () => {
                                                    try {
                                                        if (navigator.share) await navigator.share({ title: "FairPrice Gift Card", text: shareText });
                                                        else { await navigator.clipboard.writeText(shareText); alert("Gift card message copied — paste it anywhere to share."); }
                                                    } catch { /* user cancelled */ }
                                                }}
                                                className="flex-1 h-10 rounded-xl bg-indigo-600 text-white font-black text-xs flex items-center justify-center gap-2"
                                            >
