...
import { isSolar } from "@/lib/financing-utils";
import { SolarAuditForm } from "./SolarAuditForm";
import { PayslipVerificationForm } from "./PayslipVerificationForm";
...
export function FinancingOffer({ product }: FinancingOfferProps) {
    ...
    const handleApply = async (finalAuditData?: any, finalPayslipData?: any) => {
        // Step Management
        if (step === 'details') {
            if (isSolarProduct) {
                setStep('solar-audit');
            ...
            if (type === 'individual') {
                setStep('payslip-verification');
                return;
            }
        }

        if (step === 'solar-audit' && type === 'individual' && !finalPayslipData && !payslipData) {
            setStep('payslip-verification');
            return;
        }
        ...
        try {
            const res = await fetch("/api/financing/apply", {
                ...
                body: JSON.stringify({
                    ...
                    auditData: finalAuditData || auditData,
                    payslipData: finalPayslipData || payslipData
                })
            });
    ...
    const renderStep = () => {
        switch (step) {
            ...
            case 'payslip-verification':
                return (
                    <PayslipVerificationForm 
                        onBack={() => setStep(isSolarProduct ? 'solar-audit' : 'details')}
                        onComplete={(data) => {
                            setPayslipData(data);
                            handleApply(auditData, data);
                        }}
                    />
                );
            default:
                        ...
                        <div className="space-y-6">
                            ...
                            >
                                {isApplying ? "Processing..." : 
                                 isSolarProduct ? "Continue to Energy Audit" : 
                                 type === 'individual' ? "Continue to Payslip Verification" : "Continue to Application"}
                            </Button>
                        </div>
                    </>
                );
