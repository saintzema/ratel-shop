...
import { CheckCircle2, UploadCloud, Loader2, Save } from "lucide-react";
import { Button } from "@/components/ui/button";
import type { ApplicantType } from "./Step1-ApplicantType";

export type DocumentUploads = Record<string, File | null>;
...
const SALARY_DOCS: DocDef[] = [
    ...
    { key: 'vendorInvoice', label: "Vendor's Invoice", hint: 'PDF or image', accept: '.pdf,.jpg,.jpeg,.png' },
];

const BUSINESS_DOCS: DocDef[] = [
    { key: 'cac', label: 'CAC Documents (Form 1 & 2)', hint: 'PDF or image', accept: '.pdf,.jpg,.jpeg,.png' },
...
export function Step3DocumentUpload({ applicantType, initialDocuments, onNext, onBack, onSaveProgress }: Props) {
    ...
    const uploadedCount = requiredDocs.filter(d => !!documents[d.key]).length;
                        ...
                        >
                            <div className="flex items-start gap-3">
                                ...
                                <div className="flex-1 min-w-0">
                                    ...
                                    {uploaded
                                        ? <p className="text-[10px] text-emerald-600 mt-0.5 truncate">{documents[doc.key]!.name}</p>
