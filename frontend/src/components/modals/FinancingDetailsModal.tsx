...
import { X, Banknote, ShieldCheck, Calculator, Info } from "lucide-react";
import { formatPrice } from "@/lib/utils";
import { Product } from "@/lib/types";
import { calculateMonthlyPayment, getVehicleDepositPercent } from "@/lib/financing-utils";
                                ...
                                <div className="bg-emerald-50/50 border border-emerald-100 rounded-2xl p-5 mb-6">
                                    <div className="grid grid-cols-2 gap-4 text-center">
                                        <div>
                                            ...
                                            <p className="text-2xl font-black text-gray-900">{formatPrice(basePrice)}</p>
                                        </div>
                                ...
                                <div className="overflow-hidden rounded-xl border border-gray-200 bg-white">
                                    <table className="w-full text-left text-sm">
                                        ...
                                        <tbody className="divide-y divide-gray-100">
                                            {[1, 2, 3].map((years, idx) => {
                                                const analysis = calculateMonthlyPayment(product.price, 'bnpl', 'foreign_used', years);
                                                return (
                                                    <tr key={years} className={idx === 0 ? "bg-emerald-50/30" : ""}>
                                                        <td className="px-4 py-3.5 font-bold text-gray-900">{years} Year{years > 1 ? 's' : ''}</td>
                                                        <td className="px-4 py-3.5 text-gray-600 text-center">{formatPrice(analysis.deposit)}</td>
                                                        <td className="px-4 py-3.5 text-right font-black text-emerald-600">{formatPrice(analysis.monthlyPayment)}<span className="text-[10px] text-gray-400 font-normal ml-1">/mo</span></td>
                                                    </tr>
                                                );
                                            })}
                                        </tbody>
