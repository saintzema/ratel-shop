...
export function PostOrderConciergeChat({ isOpen, onClose, product, orderId, order, mode = "post_order" }: PostOrderChatProps) {
    ...
    // getProductUrl(product) silently returns "/" (the homepage) when product/product.id
    // is missing — which is exactly what made "Write a Detailed Review" look like it
    // redirected nowhere useful. Fall back to the order page instead of the homepage
    // when we don't have a resolved product to link to.
    const reviewHref = product?.id ? `${getProductUrl(product)}#reviews-section` : `/account/orders/${orderId}`;
                                        ...
                                        <div className="flex flex-col gap-1 relative cursor-pointer sm:cursor-auto" onDoubleClick={() => setReplyingTo({ sender: isUser ? 'You' : msg.sender === 'ziva' ? 'Ziva AI' : msg.sender === 'seller' ? 'Seller' : 'Admin', text: msg.text })}>
                                            ...
                                            >
                                                ...
                                                {msg.sender === "ziva" && msg.text.includes("Thanks for the") && msg.text.includes("star rating!") && (
                                                    <div className="mt-3">
                                                        <a href={reviewHref} onClick={onClose} className="block w-full">
                            ...
                            <div className="p-5 bg-white border-t border-gray-100 shrink-0 flex flex-col items-center justify-center space-y-4">
                                {reviewRating === 0 ? (
                                    ...
                                    <>
                                        ...
                                        <a href={reviewHref} onClick={onClose} className="w-full">
