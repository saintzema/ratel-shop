                            ...
                            <div key={product.id} className="group relative bg-white rounded-xl border border-gray-100 hover:border-emerald-200 hover:shadow-md transition-all overflow-hidden">
                                <Link href={`/product/${product.id}`} className="block">
                                    <div className="bg-gray-50 rounded-t-xl aspect-square p-3 flex items-center justify-center relative">
                                        <img src={(product.image_url && !product.image_url.toLowerCase().includes("no photo") && !product.image_url.toLowerCase().includes("no image") && product.image_url.toLowerCase() !== "n/a") ? product.image_url : "/assets/images/placeholder.png"} alt={product.name} className="w-full h-full object-contain mix-blend-multiply transition-transform group-hover:scale-105" onError={e => { (e.target as HTMLImageElement).src = "/assets/images/placeholder.png"; }} />
                                        {discount > 0 && (
