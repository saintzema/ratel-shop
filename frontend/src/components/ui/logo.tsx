...
export function Logo({ className, variant = "light", hideTextMobile = false, isAdmin = false }: LogoProps) {
    return (
        <Link href={isAdmin ? "/admin/dashboard" : "/"} className={cn("flex items-center gap-2 group", className)}>
            ...
            <div className={cn("flex flex-col", hideTextMobile ? "hidden md:flex" : "flex")}>
                ...
                <span className={cn("text-[8.5px] font-black uppercase tracking-widest mt-0.5", variant === "light" ? "text-brand-orange" : "text-emerald-700")}>
